...
import { ENEMIES, getRandomEnemy, getZoneRandomEnemy } from '../data/enemies.js';
...
export class RoomGenerator {
  ...
  generateRoom(type = null, playerStartPos = null, zoneType = 'green', progressionColor = null, exitLetter = null) {
    ...
    const room = {
      ...
      collisionMap: this.createCollisionMap(type, zoneType),
  ...
  spawnCrows(room, { hoardPool = null, guaranteedItem = null, count = 3 } = {}) {
    ...
    this._shuffleArray(candidates); // shuffle for variety
    ...
    while (positions.length < COUNT) {
      ...
      const tooClose = positions.some(p =>
        (p.x - candidate.x) ** 2 + (p.y - candidate.y) ** 2 < minDistFromOther ** 2);
  ...
  createCollisionMap(roomType = ROOM_TYPES.COMBAT, zoneType = 'green') {
    ...
    if (this.currentLetterTemplate?.vaultStructure?.enabled) {
      this.placeVaultStructure(map, zoneType);
    }
  ...
  generateCombatRoom(room) {
    ...
    const enemyCount = Math.max(1, Math.ceil(Math.min(1 + Math.floor(this.currentDepth / 2), 6) / 2));
  ...
  generateBossRoom(room) {
    ...
    // skip miniboss content entirely so it isn't spawned on top of the zone
    // boss. Otherwise pick from the zone's undefeated bossPool, the generic
    // fallback boss, or a defensive plain-combat spawn — see roomFeatures.js.
    if (!this.isZoneBossRoom) {
  ...
  generateUndergroundRoom(room) {
    ...
    // Sinkhole shortcut: force a guaranteed lake + river trail + arrival spawn
    // when this room was force-generated by SinkholeSystem's cross-zone
    // hand-off, rather than reached via a normal exit.
    if (room.zone === 'yellow' && this._forceSinkholeArrival) {
      this._forceSinkholeArrival = false;
      injectSinkholeLake(this, room);
    }
  ...
  spawnEnemiesFrom(game, spawner, spawnData) {
    const newEnemies = [];
    const { spawnChar, spawnCount, spawnRange, spawnerPosition, exactPosition, dataOverride } = spawnData;
    ...
    for (let i = 0; i < spawnCount; i++) {
      ...
      if (spawnPos) {
        const newEnemy = new Enemy(spawnChar, spawnPos.x, spawnPos.y, game.currentDepth, dataOverride || null);
  ...
  placeVaultStructure(collisionMap, zoneType = 'green') {
    ...
    // Per-zone unlock method (K room retired 2026-09-20 — see
    ...
    // Green key rock / cyan buried switch (if this zone's method calls for
    // one) — staged by buildVaultUnlockExtras above.
    this.pendingVaultLoot.push(...extraLoot);
