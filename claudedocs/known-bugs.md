...
| # | Bug | Source | Status |
|---|-----|--------|--------|
| 88 | **Held-item update runs twice per EXPLORE frame — all weapon timing is 2× data values; hammer shockwaves intermittently dropped** — `updatePlayerMechanics` (called by all three states) contains the staff-block-clear + `heldItem.update(deltaTime)` → windup-attack pipeline, and `updateExploreState` repeats the same block verbatim (~line 4092). In EXPLORE both run, so `Item.update` decrements `cooldownTimer`/windup/charge twice per frame: every cooldown, reload, and windup in combat is effectively half its `items.js` value (REST/NEUTRAL tick at 1×). The duplicate block also lacks the `triggerShockwave` branch — whichever copy crosses the windup threshold consumes the attack, so hammer shockwaves fire only when the first copy wins (timer-phase dependent, ~coin flip). ⚠️ Fix is balance-critical: removing the duplicate halves effective attack speed game-wide; weapons were tuned under 2× ticking. Also skews the playtesting simulator's TTK model (it uses raw data cooldowns). Decide intended timing first, then fix + retune in one pass. | refactor audit 2026-06-09 | open |
