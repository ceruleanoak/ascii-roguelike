...
export class BossRenderer {
  constructor(renderer) {
    ...
    // The green dungeon boss draws on its own sub-cell pitch around a moving
    // root, not on the arena cell grid these composites share, so it lives in
    // its own file rather than as a fourth composite here.
    this.hoardmaw = new HoardmawRenderer(renderer);
  }
  ...
  renderBossComposite(game) {
    ...
    if (game.dungeonBossSystem?.hoardmaw) { this.hoardmaw.render(game); return; }
    if (!bs?.active) return;
