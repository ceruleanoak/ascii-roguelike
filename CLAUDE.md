...
- **Run-scoped state declares its reset home at creation.** Any new `game.*` field that should die on death/title/room-exit must be cleared by the appropriate reset path (`enterTitleState`, `_resetRunToRest`, `enterRestState`, `InteriorManager.reset`) in the same commit, and verified with `node tools/check-reset-parity.mjs`. Undeclared fields are how #198/#196/#100/#86 happened.
