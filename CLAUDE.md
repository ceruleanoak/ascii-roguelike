...
- `npm run check:data` — static data-integrity gate (`tools/check-data.js`; runs inside `build`): duplicate/shadowed registry keys, affinity/dropTable/recipe referential integrity, rarity-weight zeroing. Pre-existing debt is allowlisted warns with bug refs; NEW violations fail the build.
- `node tools/check-reset-parity.mjs` — headless reset-contract harness: drives the real Game through REST→EXPLORE with pickup dirt, applies each full-reset path, diffs contract-surface state against a virgin run. Run after touching any reset path or adding run-scoped state (~6s).
- `node tools/audit-data-fields.mjs` — report-only scan for authored-but-never-read data fields (#201 shape). Verify hits by hand; dynamic access is invisible to it.
...
Recurring bug families each get one standing rule. All are enforced by tooling where possible (`npm run check:arch` / `check:data`, `tools/check-reset-parity.mjs`).

- **Reset-scoped state declares itself once, in the Reset Registry.** Any new `game.*`/`player.*` field that should die on room-exit/death/title-return gets one entry in `RESET_REGISTRY` (`src/game/resetRegistry.js`), at its narrowest correct Reset Scope (`room`/`run`/`title` — see GLOSSARY.md), in the same commit. `enterTitleState()`, `_resetRunToRest()`, `applyRoomSwap()`, and the `Game` constructor all consume the registry via `applyReset()`/`initRegisteredState()` — do not add a bespoke clear inside any of them for a field the registry should own. A system-owned reset method (e.g. `InteriorManager.reset()`) keeps its own internal logic; register its *call site* with a `covers` list instead of duplicating its fields as separate entries. Verify with `node tools/check-reset-parity.mjs`, which hard-fails on an unresolvable registry path or a live field missing from the registry/allowlist. Undeclared fields are how #198/#196/#100/#86/#13/#307 happened.
- **Never mutate the shared data registries.** `ENEMIES`/`ITEMS`/`INGREDIENTS` entries are read-only at runtime; per-instance variation goes on the instance (or a clone), and readers decide explicitly whether a flag lives on `data` or the instance (#170/#215 family). The data gate catches new key collisions and unresolvable references — don't silence it without a bug reference.
- **Independent input listeners route through `isInputCaptured(game)`** (`src/game/inputCapture.js`) instead of re-enumerating suppression gates (#142 drift shape).
- **New world-collection rendering ships both passes same-commit**: a shared `drawX(game, interior)` helper consumed by `ExploreRenderer` AND the interior PiP overlay (#110/#111/#113/#135 family). A collection renderable on only one plane says so in its helper comment.
- **NPC/companion movement uses `steerToward()`** (`src/systems/npcSteering.js` — stuck detection + detour + BFS fallback), not bare straight-line velocity, unless the movement deliberately terminates another way (#208 wedge family). Escape paths that self-terminate (exit-despawn, scatter jitter) may keep their own steering.
- **New combat/weapon tickers declare their REST behavior at wiring time.** Per-state dispatch lists in main.js mean net-new systems default to EXPLORE-only silently. Either wire through the state-shared paths (`updatePlayerMechanics`, WorldEffectsSystem) or leave a `// rest-parity: absent because …` note at the EXPLORE call site. REST's `combatSystem.update(..., [], [])` empty-list call degrades any world-context-dependent resolution ([rest-parity], bug #220).
...
5. **Budgets are enforced.** `npm run build` runs `tools/check-architecture.js` against `tools/arch-budgets.json`. The check exists to force a real look for an extraction candidate, not to be an absolute ceiling — a failure is a routing signal, not a line-count problem.
   - **Overages of ~300 chars or fewer aren't worth hunting for an extraction candidate.** `tools/check-architecture.js` has a built-in `OVERAGE_TOLERANCE`: a miss within that band prints `warn` and doesn't fail the build. This is a property of the checker itself, not something to reproduce by hand — never hand-edit `tools/arch-budgets.json`'s stored number to paper over an overage; the budget only ever moves down, via `--update` after a real extraction shrinks the file. The extraction requirement below is for overages past the tolerance.
